...
<c:choose>
 ...
 <c:otherwise>
	<c:forEach items="${textos}" var="linha" varStatus="status">
     <ul id="resultlist">
      <li>
        <a href="/infoJogo.jogo.action?id=<c:out value="${linha.jogoId}"/>">
         [<c:out value="${linha.nome}"/>]
        </a>
        - <a href="/user/profile/<c:out value="${linha.usuario.id}"/>.games">
         <c:out value="${linha.usuario.nome}"/>
        </a>
         <a href="/admin.solicitacaoAlteracaoJogoDetalhe.action?id=<c:out value="${linha.id}"/>">
          <mtw:i18n key="solicitacaoAjudaJogo.alterar"/>
         </a>
      </li>
     </ul>
