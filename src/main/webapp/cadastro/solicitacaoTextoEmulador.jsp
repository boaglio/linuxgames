...
<c:choose>
 ...
 <c:otherwise>
	<c:forEach items="${textos}" var="linha" varStatus="status">
     <ul id="resultlist">
      <li>
        <a href="/infoEmulador.inst.action?id=<c:out value="${linha.id}"/>">
         [<c:out value="${linha.nome}"/>] 
        </a> 
        - <b>
         <c:choose>
          <c:when test="${linha.texto.tipo==1}"> <mtw:i18n key="solicitacaoTextoEmulador.tipo.1"/></c:when>
          <c:when test="${linha.texto.tipo==2}"> <mtw:i18n key="solicitacaoTextoEmulador.tipo.2"/></c:when>
          <c:when test="${linha.texto.tipo==3}"> <mtw:i18n key="solicitacaoTextoEmulador.tipo.3"/></c:when>
         </c:choose>       
         </b>
        - <a href="/user/profile/<c:out value="${linha.usuario.id}"/>.games">
         <c:out value="${linha.usuario.nome}"/> (<c:out value="${linha.usuario.email}"/>)  
        </a>- 
         <a href="/admin.solicitacaoTextoEmuladorDetalhe.action?id=<c:out value="${linha.texto.id}"/>">
          <mtw:i18n key="solicitacaoTextoEmulador.alterar"/>
         </a>
      </li>
     </ul>
