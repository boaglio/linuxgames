...
<c:choose>
 ...
 <c:otherwise>
	<c:forEach items="${textos}" var="linha" varStatus="status">
     <ul id="resultlist">
      <li>
        <a href="/info.jogoInst.action?id=<c:out value="${linha.id}"/>">
        ...
        - <a href="/user/profile/<c:out value="${linha.usuario.id}"/>.games">
         ...
         <a href="/admin.solicitacaoRoteiroJogoDetalhe.action?id=<c:out value="${linha.texto.id}"/>">
          <mtw:i18n key="solicitacaoRoteiroJogo.alterar"/>
         </a>
      </li>
     </ul>
